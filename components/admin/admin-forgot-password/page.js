...
export default function AdminForgotPasswordPage() {
  ...
  const handleRequestCode = async (e) => {
    ...
    if (res.ok) { setStep(2); setSuccess(json.message || "Code generated!"); }
        ...
        <div className="admin-card space-y-4">
          ...
          {step === 1 && (
            <form onSubmit={handleRequestCode} className="space-y-4">
              <p className="text-sm text-gray-500">Click below to generate a one-time reset code. The code will appear in your Vercel function logs.</p>
              <button type="submit" disabled={loading} className="btn-primary w-full justify-center py-3">
                {loading ? <><Loader2 size={16} className="animate-spin" /> Generating...</> : "Generate Reset Code"}
              </button>
            </form>
          )}
          ...
          {step === 2 && (
            <form onSubmit={handleReset} className="space-y-4">
              <div>
                ...
                <input type="text" value={code} onChange={(e) => setCode(e.target.value)} required placeholder="Enter 6-digit code from Vercel logs" className="input-field" />
              </div>
              <div>
